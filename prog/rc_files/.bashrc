...
alias show_trash='ls ~/.local/share/Trash/files/'
alias delete_trash='sudo rm -r ~/.local/share/Trash/files/*'
alias ..='cd ../'
alias ~='cd ~'
alias cd..='cd ../'
...
alias ssh_krueger="ssh contesini@134.117.64.164"
...
function reference_ssh() {
  ...
  echo "ssh -X contesini@134.117.64.164"
  echo "sshfs contesini@134.117.64.164:/home/contesini ~/x3_kueger/"
  echo "umount x3_kueger/"
}

function fetch_data_krueger () {
...
function reference_screen(){
  ...
  echo -e "\t Ctrl-a Ctrl-c : Create a new window inside the screen session"
  echo -e "\t Ctrl-a Ctrl-n : Cycle through windows"
  echo -e "\t Ctrl-a Ctrl-d : Detach from screen (keep running in the background)"
  echo -e "\t screen -ls"
  echo -e "\t screen -r session_id"
  echo -e "\t Ctrl-a Ctrl-k : Close the current window"
  echo -e "\n"
  echo -e "------------------------------------------------------------------------------"
  echo -e "\n"
  echo -e "\t Ctrl-*: Within the screen session ..."
  echo -e "\t [ : Enter copy mode for scrolling and selecting text"
  echo -e "\t ] : Paste copied text inside the session"
  echo -e "\t A : Rename the current window"
  echo -e "\t c : Create a new window inside the screen session"
  echo -e "\t d : Detach the current session (keep running in the background)"
  echo -e "\t k : Close (kill) the current window"
  echo -e "\t n : Switch to the next window"
  echo -e "\t p : Switch to the previous window"
  echo -e "\t Q : Close all split windows except the current one"
  echo -e "\t S : Split the window horizontally"
  echo -e "\t X : Close the active split window"
...
function retrieve_from_trash () {
  mv ~/.local/share/Trash/files/"$1" "$2"
}
